...
const textDragHandle = $('#textOverlayDragHandle');
...
function render() {
  ...
  const isBeingEdited = state.interaction && state.interaction.editingTextId === selected?.id;
  if (selected && !isBeingEdited) drawSelectionHandles(selected);
...
function wrapTextLines(text, maxWidth) {
  // ctx.font must already be set by the caller before calling this, since
  // wrapping depends on measureText(). Explicit newlines are hard breaks;
  // each resulting paragraph is then soft-wrapped to fit maxWidth, matching
  // (closely enough) what the editing <textarea> overlay already shows the
  // user while they type -- before this, the canvas render never wrapped
  // at all, so committed text could silently look nothing like the preview.
...
function drawText(obj) {
  const { x, y, w, h } = obj;
  ctx.font = fontStringFor(obj);
  const lines = wrapTextLines(obj.text, w);
...
function drawBlurRegion(obj, mode) {
  ...
  if (mode === 'blur') {
    // Sample a padded region around the target rect (clamped to the base
    // image's actual bounds) so the CSS blur filter has real neighboring
    // pixels to sample at the edges instead of transparent ones -- without
    // this, blur() darkens/fades the border of the region. The offscreen
    // canvas is sized to the padded region (not the crop rect) so nothing
    // gets stretched; only the true w x h center is drawn back.
    const pad = Math.ceil(obj.intensity);
    const imgW = state.baseImageEl.naturalWidth || canvas.width;
    const imgH = state.baseImageEl.naturalHeight || canvas.height;
    const sx = Math.max(0, x - pad);
    const sy = Math.max(0, y - pad);
    const sx2 = Math.min(imgW, x + w + pad);
    const sy2 = Math.min(imgH, y + h + pad);
    ...
    offCtx.drawImage(state.baseImageEl, sx, sy, sw, sh, 0, 0, sw, sh);

    // Offset accounts for asymmetric padding near the image's own edges
    // (e.g. a blur region touching x=0 can't sample padding to its left).
    const offsetX = x - sx;
    const offsetY = y - sy;
    ctx.drawImage(off, offsetX, offsetY, w, h, x, y, w, h);
    return;
  }
...
function hitTestObject(obj, px, py) {
  ...
  if (obj.type === 'freehand' || obj.type === 'highlight-free') {
    // A bbox-only test would select the object from anywhere inside its
    // bounding rectangle, including empty space nowhere near the actual
    // stroke -- e.g. any diagonal freehand line has a bbox mostly empty.
    // Test proximity to each segment of the actual path instead.
    const pts = absPointsFor(obj);
    const threshold = obj.type === 'highlight-free' ? Math.max(14, obj.strokeWidth || 18) : Math.max(8, obj.strokeWidth);
...
function onCanvasDoubleClick(evt) {
  // Text was the one annotation type with no way to fix a typo or add a
  // word after committing -- you could move/resize the box, but never
  // touch the actual words again short of deleting and starting over.
  // Double-click re-opens the same editing textarea, pre-filled, on any
...
function onPointerMove(evt) {
  ...
  if (it.mode === 'draw-box') {
    ...
    let ey = y;
    if (evt.shiftKey) {
      // Hold Shift to constrain a rectangle/circle to a perfect
      // square/circle -- standard behavior in every professional editor,
      // and there was previously no way to draw a precise square/circle
      // without eyeballing it.
      const dx = x - it.startX;
      const dy = y - it.startY;
      const size = Math.max(Math.abs(dx), Math.abs(dy));
      ex = it.startX + Math.sign(dx || 1) * size;
      ey = it.startY + Math.sign(dy || 1) * size;
    }
    it.previewObject = buildPreviewObject(state.tool, it.startX, it.startY, ex, ey);
    render();
    return;
  }
  ...
  if (it.mode === 'draw-line') {
    ...
    let ey = y;
    if (evt.shiftKey) {
      // Hold Shift to snap a line/arrow to the nearest 45° increment
      // (perfectly horizontal, vertical, or diagonal) -- there was
      // previously no way to draw a precisely straight line by hand.
      const dx = x - it.startX;
      const dy = y - it.startY;
      const dist = Math.hypot(dx, dy);
      const angle = Math.atan2(dy, dx);
      const snapAngle = Math.round(angle / (Math.PI / 4)) * (Math.PI / 4);
      ex = it.startX + Math.cos(snapAngle) * dist;
      ey = it.startY + Math.sin(snapAngle) * dist;
    }
    it.previewObject.x1 = it.startX;
    it.previewObject.y1 = it.startY;
    it.previewObject.x2 = ex;
    it.previewObject.y2 = ey;
    render();
    return;
  }
...
function applyResize(obj, orig, handle, mx, my) {
  ...
  // Every other object type visibly scales when resized -- without this,
  // dragging a text box's corner handle changed the box but left the text
  // itself the same size, which reads as broken rather than intentional.
  // Only handles that touch height (n/s/corners) scale the font, so a
  // pure width drag (e/w only) can still be used just to change the wrap
  // width without also resizing the text.
  if (obj.type === 'text' && orig.h > 0 && (handle.includes('n') || handle.includes('s'))) {
    const scale = h / orig.h;
    obj.fontSize = Math.max(8, Math.min(400, Math.round((orig.fontSize || obj.fontSize) * scale)));
  }
}
...
function syncTextOverlayStyle(obj) {
  const rect = canvas.getBoundingClientRect();
  ...
  // The CSS `font` shorthand resets `line-height` to `normal` unless a
  // line-height is explicitly included via the `size/line-height` syntax
  // -- without the "/1.25" here, this silently clobbered the stylesheet's
  // `line-height: 1.25` rule every time this ran, undoing the fix for the
  // textarea/canvas line-height mismatch.
  textOverlay.style.font = `${obj.italic ? 'italic ' : ''}${obj.bold ? '700 ' : ''}${obj.fontSize * state.zoom}px/1.25 ${obj.fontFamily}`;
  textOverlay.style.color = obj.color;
  textOverlay.style.textAlign = obj.align;
  textOverlay.style.textDecoration = obj.underline ? 'underline' : 'none';
  textOverlay.style.background =
    obj.bgOpacity > 0 ? hexToRgba(obj.bgColor, obj.bgOpacity) : 'transparent';

  // Sits just outside the textarea's own top-left corner so it never
  // overlaps the text-selection/cursor area, but stays visually attached.
  textDragHandle.style.left = left - 11 + 'px';
  textDragHandle.style.top = top - 11 + 'px';
}

let textOverlayResizeObserver = null;

function openTextEditor(obj, isNew) {
  syncTextOverlayStyle(obj);
  textOverlay.value = obj.text || '';
  textOverlay.classList.remove('ed-hidden');
  textDragHandle.classList.remove('ed-hidden');
  textOverlay.focus();
  if (!isNew) textOverlay.select();

  state.interaction = { editingTextId: obj.id };
  render();

  // Users can drag the textarea's own native resize handle (bottom-right
  // corner) to change the text box's size while actively typing, instead
  // of only being able to resize after committing. Keep obj.w/h in sync
  // continuously so the wrap width and font-scaling-on-commit both use
  // whatever size the user actually leaves it at.
  if (textOverlayResizeObserver) textOverlayResizeObserver.disconnect();
  let lastW = obj.w;
  let lastH = obj.h;
  textOverlayResizeObserver = new ResizeObserver(() => {
    const newW = textOverlay.offsetWidth / state.zoom;
    const newH = textOverlay.offsetHeight / state.zoom;
  ...
  textOverlayResizeObserver.observe(textOverlay);

  // Small grip handle above the box's corner lets you reposition it while
  // still actively typing, instead of having to commit first, switch to
  // the Select tool, and drag it. mousedown's preventDefault() is what
  // stops the browser's default "clicking anything else blurs the
  // focused textarea" behavior from ending the edit the moment you try
  // to drag -- without it, grabbing the handle would immediately commit
  // and close the editor before the drag could even start.
  let dragOrigin = null;
  const onDragMouseDown = (e) => {
    e.preventDefault();
    dragOrigin = { mouseX: e.clientX, mouseY: e.clientY, objX: obj.x, objY: obj.y };
    window.addEventListener('mousemove', onDragMouseMove);
    window.addEventListener('mouseup', onDragMouseUp);
  };
  const onDragMouseMove = (e) => {
    if (!dragOrigin) return;
    const dx = (e.clientX - dragOrigin.mouseX) / state.zoom;
    const dy = (e.clientY - dragOrigin.mouseY) / state.zoom;
    obj.x = dragOrigin.objX + dx;
    obj.y = dragOrigin.objY + dy;
    syncTextOverlayStyle(obj);
  };
  const onDragMouseUp = () => {
    dragOrigin = null;
  ...
  textDragHandle.addEventListener('mousedown', onDragMouseDown);

  const commit = () => {
    if (textOverlayResizeObserver) {
      textOverlayResizeObserver.disconnect();
      textOverlayResizeObserver = null;
    }
    textDragHandle.removeEventListener('mousedown', onDragMouseDown);
    window.removeEventListener('mousemove', onDragMouseMove);
    window.removeEventListener('mouseup', onDragMouseUp);
    textDragHandle.classList.add('ed-hidden');
    ...
    state.interaction = null;
    if (!obj.text.trim()) {
      // Applies whether this was a brand new box or an existing one that
      // got fully cleared out while re-editing -- an empty text box left
      // behind either way is just clutter, not something worth keeping.
    ...
    // Every other annotation tool (rect, line, freehand, ...) returns to
    // the Select tool right after committing so the very next click
    // manipulates what was just created instead of starting a brand new
    // shape. Text was the one tool that didn't do this -- clicking
    // anywhere after typing would silently start placing ANOTHER text box
    // instead of letting you select/move/resize the one you just made.
    setActiveTool('select');
  };
...
function measureTextHeight(obj) {
  ctx.font = fontStringFor(obj);
  const lines = wrapTextLines(obj.text, obj.w);
  return Math.max(1, lines.length) * obj.fontSize * 1.25 + 8;
}
...
function applyLivePropertyChange() {
  ...
  if (selected.type === 'text') {
    Object.assign(selected, currentTextStyle());
    // If this text object is the one currently being edited, the visible
    // "canvas" for it right now IS the textarea overlay (render() skips
    // drawing an object mid-edit) -- without this, adjusting font size,
    // color, bold, etc. from the side panel while still typing would
    // silently do nothing the user could see until they clicked away.
    if (state.interaction && state.interaction.editingTextId === selected.id) {
      syncTextOverlayStyle(selected);
    }
