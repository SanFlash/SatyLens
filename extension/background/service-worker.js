...
async function openEditorFor(id) {
  const url = chrome.runtime.getURL(`editor/editor.html?id=${encodeURIComponent(id)}`);
  await chrome.tabs.create({ url });
}
...
// Set by START_AREA_SELECT and read back when AREA_SELECTED arrives --
// the drag-to-select overlay itself (selector.js) is shared between the
// screenshot and recording flows; this is what tells the AREA_SELECTED
// handler which one the user actually asked for.
let pendingAreaSelectPurpose = 'screenshot';

async function startAreaSelect(purpose) {
  pendingAreaSelectPurpose = purpose === 'record' ? 'record' : 'screenshot';
  const tab = await getActiveTab();
  await chrome.scripting
    .executeScript({ target: { tabId: tab.id }, files: ['selector/selector.js'] })
    .catch(() => {});
...
async function handleAreaSelected(message, sender) {
  if (pendingAreaSelectPurpose === 'record') {
    await handleAreaSelectedForRecording(message, sender);
    return;
  }
  ...
  try {
    ...
    // Select Area is the one screenshot mode that goes straight to the
    // editor instead of the gallery -- it's already saved to the gallery
    // in the background (so nothing is lost if the editor tab is closed
    // without saving again), but the point of drawing a precise region is
    // almost always "I'm about to annotate/redact this," so skip the
    // extra click of opening it from the gallery afterward.
    await notify('Screenshot captured ✅', 'Opening the editor…');
    broadcastGalleryUpdate();
    await openEditorFor(capture.id);
  } catch (err) {
...
async function handleAreaSelectedForRecording(message, sender) {
  try {
    ...
    const params = new URLSearchParams({
      mode: 'area',
      tabId: String(tab.id),
      rx: String(message.rect.left),
      ry: String(message.rect.top),
      rw: String(message.rect.width),
      rh: String(message.rect.height),
      dpr: String(dpr)
    });
    const url = chrome.runtime.getURL(`recorder/recorder.html?${params.toString()}`);
    await chrome.tabs.create({ url });
  } catch (err) {
    console.error('Area select for recording failed:', err);
    track('ERROR_OCCURRED', { feature: 'recording', action: 'select_area', success: false, error: err.message });
    await notify('❌ Could not start area recording', err.message || 'Unable to use the selected area.');
  }
}
    ...
    try {
      switch (message.action) {
        ...
        case 'START_AREA_SELECT': {
          await startAreaSelect(message.purpose);
