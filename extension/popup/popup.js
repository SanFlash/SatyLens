...
async function handleSelectArea() {
  try {
    await sendMessage({ action: 'START_AREA_SELECT', purpose: 'screenshot' });
    ...
    // and opens the editor once the user finishes their selection.
    window.close();
  } catch (err) {
    showToast(err.message, 'error');
  }
...
async function handleRecordArea() {
  try {
    await sendMessage({ action: 'START_AREA_SELECT', purpose: 'record' });
    // Same overlay as Select Area screenshots, but the background script
    // opens the recorder (pre-scoped to the drawn rectangle) once the
    // selection is confirmed, instead of capturing a screenshot.
    window.close();
  } catch (err) {
    showToast(err.message, 'error');
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-action]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'record-area') handleRecordArea();
    });
  });
